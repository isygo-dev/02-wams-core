...
public class ByokView extends VerticalLayout {
    ...
    private void generateParameters() {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                Notification.show("Parameters generated (valid until " + params.getValidTo() + ")", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            } else {
                Notification.show("Failed to get import parameters", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        } catch (Exception e) {
    ...
    private void importKeyMaterial() {
        ...
        if (!StringUtils.hasText(encryptedMaterialField.getValue())) {
            Notification.show("Encrypted key material is required", 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_WARNING);
            return;
        }
        if (currentImportToken == null) {
            Notification.show("Please generate import parameters first (they expire after 24h)", 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_WARNING);
            return;
        }
        showLoading(true);
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                Notification.show("Key material imported successfully", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                clearParameters();
            } else {
                Notification.show("Import failed", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        } catch (Exception e) {
            Notification.show("Error: " + e.getMessage(), 6000, Notification.Position.TOP_END)
    ...
    private void deleteImportedMaterial() {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                Notification.show("Imported key material deleted", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                clearParameters();
            } else {
                Notification.show("Deletion failed", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        } catch (Exception e) {
            Notification.show("Error: " + e.getMessage(), 6000, Notification.Position.TOP_END)
