...
@PermitAll
public class CustomerManagementView extends ManagementVerticalView {
    ...
    private final Button createButton = new Button(I18n.t("ims.customer.view.create.button"), new Icon(VaadinIcon.PLUS_CIRCLE));
    ...
    private List<CustomerDto> currentPageCustomers = new ArrayList<>();

    private String currentSearch = "";
    private IEnumEnabledBinaryStatus.Types currentAdminStatus = null;

    @Autowired
    public CustomerManagementView(CustomerService customerService, CustomerImageService customerImageService, AccountService accountService) {
        ...
        H2 header = new H2(I18n.t("ims.customer.view.title"));
        ...
        cardsContainer.addClassName("customer-cards-grid");
        add(cardsContainer);
    ...
    private void initEventHandlers() {
        ...
        createButton.setTooltipText(I18n.t("ims.customer.view.create.tooltip"));

        refreshButton.addClickListener(e -> loadPage(0));
        refreshButton.setTooltipText(I18n.t("ims.customer.view.refresh.tooltip"));

        searchField.setPlaceholder(I18n.t("ims.customer.view.search.placeholder"));
    ...
    private void loadPage(int page) {
        showLoading(true);
        try {
            ResponseEntity<PaginatedResponseDto<CustomerDto>> response =
                    customerService.findAll(page, pageSize);
            PaginatedResponseDto<CustomerDto> body = response.getBody();
            if (body != null && body.getContent() != null) {
                currentPageCustomers = body.getContent();
            ...
            Notification.show(I18n.t("ims.customer.view.load.error", errorMsg), 6000, Notification.Position.BOTTOM_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
            log.error("Failed to load customers", ex);
        } catch (Exception e) {
            Notification.show(I18n.t("ims.customer.view.load.error", e.getMessage()), 6000, Notification.Position.BOTTOM_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
    ...
    private void filterAndDisplayCards() {
        List<CustomerDto> filtered = currentPageCustomers.stream()
                .filter(customer -> {
                    if (currentAdminStatus != null && customer.getAdminStatus() != currentAdminStatus)
                        return false;
                    if (currentSearch != null && !currentSearch.isEmpty()) {
                        String searchLower = currentSearch.toLowerCase();
                        return (customer.getName() != null && customer.getName().toLowerCase().contains(searchLower)) ||
        ...
        if (filtered.isEmpty()) {
            ...
            for (CustomerDto customer : filtered) {
                cardsContainer.add(new CustomerCard(this, customerService, customerImageService,
                        accountService,
                        customer, this::loadPageZero));
            }
        }
    }

    private void updatePaginationDisplay() {
        pageInfoLabel.setText(I18n.t("ims.customer.view.page.info", currentPage + 1, totalPages));
        totalCountLabel.setText(I18n.t("ims.customer.view.total.count", totalElements));
    ...
    private HorizontalLayout buildToolbar() {
        ...
        toolbar.addClassName("customer-management-toolbar");
    ...
    private void openCreateCustomerDialog() {
        new CreateCustomerDialog(this, customerService, customerImageService, this::loadPageZero).open();
    }

    public void openUpdateCustomerDialog(CustomerDto customer, Runnable onSuccess) {
        new UpdateCustomerDialog(this, customerService, customerImageService, customer, onSuccess).open();
    }
