...
@PermitAll
public class ApplicationManagementView extends VerticalLayout implements BeforeEnterObserver {

    private final ApplicationService applicationService;
    private final ApplicationImageService applicationImageService;

    private final Div cardsContainer = new Div();
    private final Button createButton = new Button("Create application", new Icon(VaadinIcon.PLUS_CIRCLE));
    ...
    private List<ApplicationDto> currentPageApplications = new ArrayList<>();

    private String currentSearch = "";
    private IEnumEnabledBinaryStatus.Types currentAdminStatus = null;

    @Autowired
    public ApplicationManagementView(ApplicationService applicationService,
        ...
        H2 header = new H2("Application Management");
        ...
        cardsContainer.addClassName("application-cards-grid");
        add(cardsContainer);
    ...
    private void initEventHandlers() {
        ...
        createButton.setTooltipText("Create a new application");

        refreshButton.addClickListener(e -> loadPage(0));
        refreshButton.setTooltipText("Refresh applications from server");

        searchField.setPlaceholder("Search by name or title");
    ...
    private void loadPage(int page) {
        showLoading(true);
        try {
            ResponseEntity<PaginatedResponseDto<ApplicationDto>> response =
                    applicationService.findAll(page, pageSize);
            PaginatedResponseDto<ApplicationDto> body = response.getBody();
            if (body != null && body.getContent() != null) {
                currentPageApplications = body.getContent();
                totalElements = body.getTotalElements();
                totalPages = body.getTotalPages();
                currentPage = body.getPageNumber();
            } else {
                currentPageApplications = new ArrayList<>();
                totalElements = 0;
                totalPages = 0;
            }
            ...
            Notification.show("Failed to load applications: " + errorMsg, 6000, Notification.Position.BOTTOM_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
            log.error("Failed to load applications", ex);
        } catch (Exception e) {
            Notification.show("Failed to load applications: " + e.getMessage(), 6000, Notification.Position.BOTTOM_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
    ...
    private void filterAndDisplayCards() {
        List<ApplicationDto> filtered = currentPageApplications.stream()
                .filter(app -> {
                    if (currentAdminStatus != null && app.getAdminStatus() != currentAdminStatus)
                        return false;
                    if (currentSearch != null && !currentSearch.isEmpty()) {
                        String searchLower = currentSearch.toLowerCase();
                        return (app.getName() != null && app.getName().toLowerCase().contains(searchLower)) ||
                                (app.getTitle() != null && app.getTitle().toLowerCase().contains(searchLower));
                    }
                    return true;
                })
        ...
        if (filtered.isEmpty()) {
            ...
            H4 emptyTitle = new H4("No applications found");
            Paragraph emptyDesc = new Paragraph("Try adjusting your search or filter criteria.");
            emptyDesc.addClassName(LumoUtility.TextColor.SECONDARY);
            emptyState.add(emptyIcon, emptyTitle, emptyDesc);
            cardsContainer.add(emptyState);
        } else {
            for (ApplicationDto app : filtered) {
                cardsContainer.add(new ApplicationCard(this, applicationService, applicationImageService, app, this::loadPageZero));
            }
        }
    }
    ...
    private void updatePaginationDisplay() {
        ...
        totalCountLabel.setText(String.format("%d total applications", totalElements));
    ...
    private HorizontalLayout buildToolbar() {
        ...
        toolbar.addClassName("application-management-toolbar");
    ...
    private void openCreateApplicationDialog() {
        new CreateApplicationDialog(this, applicationService, applicationImageService, this::loadPageZero).open();
    }

    public void openUpdateApplicationDialog(ApplicationDto application, Runnable onSuccess) {
        new UpdateApplicationDialog(this, applicationService, applicationImageService, application, onSuccess).open();
    }
