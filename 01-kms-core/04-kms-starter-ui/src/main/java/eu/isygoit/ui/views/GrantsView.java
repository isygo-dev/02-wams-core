...
public class GrantsView extends VerticalLayout {
    ...
    private void loadGrants() {
        ...
        try {
            ...
            Notification.show("Failed to load grants: " + e.getMessage(), 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
            grantsGrid.setItems(new ArrayList<>());
        } finally {
            showLoading(false);
    ...
    private void openCreateGrantDialog() {
        ...
        Button createBtn = new Button("Create", e -> {
            ...
            if (!StringUtils.hasText(grantee) || operations.isEmpty()) {
                Notification.show("Grantee principal and at least one operation are required", 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
            ...
            if (StringUtils.hasText(constraintsArea.getValue())) {
                try {
                    ...
                    Notification.show("Invalid constraints JSON: " + ex.getMessage(), 6000, Notification.Position.TOP_END)
                            .addThemeVariants(NotificationVariant.LUMO_ERROR);
                    return;
                }
            }
            ...
            try {
                ...
                if (response.getStatusCode().is2xxSuccessful()) {
                    Notification.show("Grant created", 6000, Notification.Position.TOP_END)
                            .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                    loadGrants();
                } else {
                    Notification.show("Creation failed: " + response.getStatusCode(), 6000, Notification.Position.TOP_END)
                            .addThemeVariants(NotificationVariant.LUMO_ERROR);
                }
            } catch (Exception ex) {
                Notification.show("Error: " + ex.getMessage(), 6000, Notification.Position.TOP_END)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });
    ...
    private void revokeSelectedGrant() {
        ...
        if (selected == null) {
            Notification.show("No grant selected", 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_WARNING);
            return;
        }
        if (selectedKeyId == null) return;
        try {
            ...
            Notification.show("Grant revoked", 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            loadGrants();
        } catch (Exception e) {
            Notification.show("Failed to revoke grant: " + e.getMessage(), 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
        }
    }
    ...
    private void retireSelectedGrant() {
        ...
        try {
            ...
            Notification.show("Grant retired", 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            loadGrants();
        } catch (Exception e) {
            Notification.show("Failed to retire grant: " + e.getMessage(), 6000, Notification.Position.TOP_END)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
        }
    }
