package eu.isygoit.ui.views.secrets.config.password;
...
import eu.isygoit.ui.views.secrets.config.password.dialog.CreatePasswordConfigDialog;
...
@Route(value = "password-configs", layout = MainLayout.class)
...
public class PasswordConfigView extends VerticalLayout {
    ...
    private List<PasswordConfigDto> currentPageContent = new ArrayList<>();
    private String currentSearch = "";

    @Autowired
    public PasswordConfigView(PasswordConfigService configService) {
    ...
    private void buildUI() {
        ...
        H2 header = new H2("Password Configurations");
    ...
    private HorizontalLayout buildToolbar() {
        ...
        toolbar.addClassName("password-toolbar");
    ...
    private void loadConfigs() {
        showLoading(true);
        try {
            ResponseEntity<PaginatedResponseDto<PasswordConfigDto>> response;
            ...
            PaginatedResponseDto<PasswordConfigDto> body = response.getBody();
            if (body != null) {
    ...
    private void renderCards() {
        ...
        if (currentPageContent.isEmpty()) {
            ...
            Paragraph emptyDesc = new Paragraph("Click 'Create Config' to add a password configuration.");
        ...
        for (PasswordConfigDto dto : currentPageContent) {
            PasswordConfigCard card = new PasswordConfigCard(this, configService, dto, () -> loadConfigs());
            cardsContainer.add(card);
        }
    }

    public void refreshCard(PasswordConfigCard card) {
        try {
            ResponseEntity<PasswordConfigDto> response = configService.findById(card.getDto().getId());
            PasswordConfigDto updated = response.getBody();
            if (updated != null) {
    ...
    private void openCreateDialog() {
        new CreatePasswordConfigDialog(configService, this::loadConfigs).open();
    }
