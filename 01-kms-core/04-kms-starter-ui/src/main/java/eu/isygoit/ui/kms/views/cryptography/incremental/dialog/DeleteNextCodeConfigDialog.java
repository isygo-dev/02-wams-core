...
public class DeleteNextCodeConfigDialog extends PinBaseActionDialog {
    ...
    public DeleteNextCodeConfigDialog(KmsAppNextCodeService nextCodeService,
                                      ...
                                      String attribute,
                                      Runnable onSuccess) {
        super(I18n.t("delete.nextcode.dialog.title"),
                I18n.t("delete.nextcode.dialog.message", entity, attribute),
                onSuccess,
                true); // require PIN
        this.nextCodeService = nextCodeService;
        this.configId = configId;
        setOkButtonText(I18n.t("delete.nextcode.dialog.button"));
    ...
    protected boolean onOk() {
        if (!validatePin()) {
            append(I18n.t("delete.nextcode.dialog.invalid.code"));
            return false;
        }

        try {
            nextCodeService.delete(configId);
            append(I18n.t("delete.nextcode.dialog.success"));
            return true;
        } catch (Exception e) {
            append(I18n.t("delete.nextcode.dialog.failed", e.getMessage()));
            return false;
        }
    }
}